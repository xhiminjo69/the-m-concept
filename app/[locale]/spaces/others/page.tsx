...
export default async function OthersPage() {
  ...
  const space = await getSpaceByCategory('others');
  ...
  return (
    <>
      ...
      <section
        ...
        style={{ paddingTop: '7rem', paddingBottom: '5rem', backgroundImage: 'url(/images/Projects-Residential.jpg)', backgroundSize: 'cover', backgroundPosition: 'center' }}
      >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[200px,1fr] gap-x-20 gap-y-8 items-end">
            <div>
              ...
              <p className="font-body text-stone-300 leading-relaxed" style={{ fontSize: '0.875rem' }}>
                {t('categories.others.heroDescription')}
              </p>
            </div>
            <div>
              <h1
      ...
      {images.length > 0 && (
        <section style={{ padding: 'clamp(5rem, 8vw, 9rem) 0', background: 'linear-gradient(to bottom, #CCC6B8 0%, #EAE7E2 22%)' }}>
          <div className="px-6 md:px-10 lg:px-16">
            <GalleryGrid images={images} projectName={t('categories.others.label')} />
          </div>
        </section>
      )}
